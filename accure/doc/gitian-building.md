...
*Setup instructions for a Gitian build of Accure Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Accure
...
- [Building Accure Core](#building-accure-core)
...
Clone the git repositories for accure and Gitian.
...
in the accure repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Accure Core
----------------

To build Accure Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the accure repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/accure/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/accure-project/accure
...
For example:
```bash
URL=https://github.com/thrasher-/accure.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit accure=${COMMIT} --url accure=${URL} ../accure/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit accure=${COMMIT} --url accure=${URL} ../accure/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit accure=${COMMIT} --url accure=${URL} ../accure/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the accure git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../accure/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/accure
SIGPATH=/some/root/path/accure-detached-sigs

./bin/gbuild --url accure=${BTCPATH},signature=${SIGPATH} ../accure/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/accure-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/accure-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/accure-osx-build.assert
```
...
[accure-project/gitian.sigs.ltc](https://github.com/accure-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
